  ...
  try {
    ...
    if (referralRequest.status !== ReferralRequestStatus.PENDING) {
      return jsonResponse({
        statusCode: 409,
        body: {
          message: "Referral is in an invalid state, cannot be verified",
        },
      });
    }
    ...
    console.error("Error verifying referral request", error);
    return jsonResponse({
      statusCode: 500,
      body: {
        message: "Error verifying referral request",
      },
    });
  }
};
